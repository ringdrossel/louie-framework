...
# Pi discovers project-local prompt templates at .pi/prompts/<name>.md
# (markdown drop-ins, invoked as /<name>). This is the equivalent of
# Claude Code's .claude/commands/. Pi's other slash-command mechanism
# requires TypeScript extensions and is overkill for LOUIE's needs.
mkdir -p "$LOUIE_DIR/.pi/prompts"

count=0
for cmd in "$COMMANDS_SRC"/louie-*.md; do
  if [ -f "$cmd" ]; then
    cp "$cmd" "$LOUIE_DIR/.pi/prompts/"
    count=$((count + 1))
...
| `/louie-status` | Read-only project snapshot (features, open questions, stale docs) |
